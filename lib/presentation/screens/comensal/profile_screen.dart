...
void _dialogoSoporte(BuildContext context, {int? establecimientoId}) {
  ...
  showDialog(
    context: context,
    barrierDismissible: false, // Evita cerrar mientras se envía
    builder: (dialogContext) => StatefulBuilder(
      builder: (context, setDialogState) {
        return AlertDialog(
              ...
              onPressed: isSending
                  ...
                  : () async {
                      ...
                      if (texto.isEmpty || texto.length < 10) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Escribe una descripción de al menos 10 caracteres.'),
                      ...
                      try {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Reporte enviado con éxito. Gracias por tu ayuda.'),
                            backgroundColor: Colors.green,
                          ),
                        );
                      } catch (e) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Error al enviar el reporte: ${e.toString().replaceAll("Exception: ", "")}'),
